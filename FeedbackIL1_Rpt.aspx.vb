...
Partial Class FeedbackIL1_Rpt
    Inherits System.Web.UI.Page
    'Create by Manoj Kumar (OSJ1874) Date : 21-05-2021
    'Feedback report for IL1 for his score card
    ...
    Private Sub FeedbackIL1_Rpt_Init(sender As Object, e As EventArgs) Handles Me.Init
        loadLoggedInUserIDAndDomainIntoSession()
    End Sub
    Private Sub FeedbackIL1_Rpt_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Public Function ChkRole() As Boolean
        Try
            ...
            cmd.CommandText += " and IGP_STATUS ='A' and IGP_user_id=:userId"
            cmd.Parameters.Clear()
            cmd.Parameters.AddWithValue("userId", Session("ADM_USER").ToString())
            Dim f = getRecordInDt(cmd)
            If f.Rows.Count > 0 Then
                t = True
            Else
                t = False
        ...
        End Try
    End Function
    Public Function getRecordInDt(ByVal cmd1 As OracleCommand) As DataTable
    ...
    Protected Sub lbtnView_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbtnView.Click
        Try
            ...
            If txt_Pno.Text <> "" Then
                ...
                qry.CommandText = "select decode(upper(ss_categ),upper('Self'),'Self',upper('MANGR'),'Manager',upper('PEER'),'Peers',upper('intsh') "
                qry.CommandText += ",'Internal stakeholders',upper('ropt'),'Subordinates') ss_categ,sum(ss_q1_a),sum(ss_q1_b),sum(ss_q1_c),sum(ss_q1_d),round(sum(ss_q1_a)/count(*),2) A,"
                qry.CommandText += "round(sum(ss_q1_b)/count(*),2) B,round(sum(ss_q1_c)/count(*),2) C,round(sum(ss_q1_d)/count(*),2) D"
                qry.CommandText += ",count(*) No_Records,upper(ss_categ) categ from t_survey_status a,tips.t_empl_all b where a.ss_asses_pno=b.ema_perno and ss_asses_pno =:pno  and ss_wfl_status ='3' and ss_year= :fy and ss_status = 'SE' and ss_del_tag = 'N' and ss_app_tag = 'AP' and b.ema_eqv_level='I1' and upper(ss_categ) not in ('PEER','ROPT','SELF','INTSH')  group by ss_categ"
                qry.Parameters.Clear()
                qry.Parameters.AddWithValue("pno", txt_Pno.Text)
                qry.Parameters.AddWithValue("fy", ViewState("FY").ToString)
                Dim re As String = String.Empty
                Dim cont As Integer = 0
                Dim dMinScore = getRecordInDt(qry)
                If dMinScore.Rows.Count > 0 Then
                    If dMinScore.Rows.Count < 1 Then
                        cntMinFullFill = 0
                    Else
                        For i As Integer = 0 To dMinScore.Rows.Count - 1
                            ...
                            '    cntMinFullFill = 0
                            If dMinScore.Rows(i)("categ").ToString = "MANGR" And Val(dMinScore.Rows(i)("No_Records").ToString) < 1 Then
                                cntMinFullFill = 0
    ...
    Public Sub GetFy()
        Try
            ...
            s1.Connection = conHrps
            Dim f1 = getRecordInDt(s1)
            ...
            Else
                ...
                s.Connection = conHrps
                Dim f = getRecordInDt(s)
    ...
    Public Sub getQ2AresCoupt(pno As String)
        Try
            Dim qry As New OracleCommand()
            qry.CommandText = "select ss_q2_a from t_survey_status  where ss_asses_pno=:pno and ss_year=:fy and upper(ss_categ) = upper('self') and ss_q2_a is not null and ss_wfl_status='3'"
            qry.Connection = conHrps
            qry.Parameters.Clear()
            qry.Parameters.AddWithValue("pno", pno.ToString)
            qry.Parameters.AddWithValue("fy", ViewState("FY").ToString)
            Dim re As String = String.Empty
            Dim cont As Integer = 0
            Dim dt As DataTable
            dt = getRecordInDt(qry)

            Dim qry1 As New OracleCommand()
            qry1.CommandText = "select ss_q2_a from t_survey_status  where ss_asses_pno=:pno and ss_year=:fy and upper(ss_categ) <> upper('self') and ss_q2_a is not null and ss_wfl_status='3' "
            qry1.Connection = conHrps
            qry1.Parameters.Clear()
            qry1.Parameters.AddWithValue("pno", pno.ToString)
            qry1.Parameters.AddWithValue("fy", ViewState("FY").ToString)
            Dim dt1 As DataTable
            dt1 = getRecordInDt(qry1)
    ...
    Public Sub getQ2BresCoupt(pno As String)
        Try
            Dim qry As New OracleCommand()
            qry.CommandText = "select ss_q2_b from t_survey_status  where ss_asses_pno=:pno and ss_year=:fy and upper(ss_categ) = upper('self') and ss_q2_b is not null and ss_wfl_status='3'"
            qry.Connection = conHrps
            qry.Parameters.Clear()
            qry.Parameters.AddWithValue("pno", pno.ToString)
            qry.Parameters.AddWithValue("fy", ViewState("FY").ToString)
            ...
            Dim qry1 As New OracleCommand()
            qry1.CommandText = "select ss_q2_b from t_survey_status  where ss_asses_pno=:pno and ss_year=:fy and upper(ss_categ) <> upper('self') and ss_q2_b is not null and ss_wfl_status='3' "
            qry1.Connection = conHrps
            qry1.Parameters.Clear()
            qry1.Parameters.AddWithValue("pno", pno.ToString)
            qry1.Parameters.AddWithValue("fy", ViewState("FY").ToString)
    ...
    Public Sub getPnoDetails(pno As String)
        ...
        qry.CommandText = "select ema_ename,ema_dept_desc from tips.t_empl_all where ema_perno=:pno and EMA_COMP_CODE='1000' and EMA_DISCH_DT is null"
        qry.Connection = conHrps
        qry.Parameters.Clear()
        qry.Parameters.AddWithValue("pno", pno.ToString)
        Dim dtPno = getRecordInDt(qry)
    ...
    Public Sub getresCoupt(pno As String)
        Try
            Dim qry As New OracleCommand()
            qry.CommandText = "select A.ss_year,A.ss_asses_pno,A.ss_categ, decode(a.ss_categ,'INTSH',3,'MANGR',1,'PEER',3,'ROPT',3,'Self',1,'SELF',1) minim,a.approved,nvl(c.completed, 0) Completed,NVL(b.rejected, 0) Insufficient_Exposure,decode(sign(a.approved - b.rejected -decode(a.ss_categ,'INTSH', 3, 'MANGR',1, 'PEER', 3,'ROPT',3)) ,'-1','LESS','OK') criteria,d.irc_desc from (select ss_year,ss_asses_pno,ss_categ,count(*) approved from hrps.t_survey_status where ss_year = :fy and ss_status = 'SE' and ss_del_tag = 'N' and ss_app_tag = 'AP' group by ss_year, ss_asses_pno, ss_categ) a,(select ss_year, ss_asses_pno, ss_categ , count(*) rejected from hrps.t_survey_status where ss_year = :fy and ss_status = 'SE'  and ss_del_tag = 'N'  and ss_app_tag = 'AP' and ss_wfl_status = 9  group by ss_year, ss_asses_pno, ss_categ) b,(select ss_year, ss_asses_pno, ss_categ, count(*) completed  from hrps.t_survey_status where ss_year = :fy and ss_status = 'SE' and ss_del_tag = 'N' and ss_app_tag = 'AP' and ss_wfl_status = 3 group by ss_year, ss_asses_pno, ss_categ) c,(select irc_code,irc_desc from t_ir_codes where  irc_type='360RL' ) d where a.ss_year = b.ss_year(+)  and a.ss_asses_pno = b.ss_asses_pno(+) and a.ss_categ = b.ss_categ(+) and a.ss_year = c.ss_year(+) and a.ss_asses_pno = c.ss_asses_pno(+) and a.ss_categ = c.ss_categ(+) and upper(a.ss_categ)=d.irc_code(+) and a.ss_asses_pno =:pno order by 1, 2, 3, 7"
            qry.Connection = conHrps
            qry.Parameters.Clear()
            qry.Parameters.AddWithValue("pno", pno.ToString)
            qry.Parameters.AddWithValue("fy", ViewState("FY").ToString)
            Dim re As String = String.Empty
            Dim cont As Integer = 0
            Dim d = getRecordInDt(qry)

            Dim qry1 As New OracleCommand()
            qry1.CommandText = "select A.ss_year,A.ss_asses_pno,nvl(sum(a.approved),0) approved,nvl(sum(c.completed), 0) Completed,NVL(sum(b.rejected), 0) Insufficient_Exposure from (select ss_year,ss_asses_pno,ss_categ,count(*) approved from hrps.t_survey_status where ss_year = :fy and ss_status = 'SE' and ss_del_tag = 'N' and ss_app_tag = 'AP'  group by ss_year, ss_asses_pno, ss_categ) a,(select ss_year, ss_asses_pno, ss_categ , count(*) rejected from hrps.t_survey_status where ss_year = :fy and ss_status = 'SE'  and ss_del_tag = 'N'  and ss_app_tag = 'AP' and ss_wfl_status = 9  group by ss_year, ss_asses_pno, ss_categ) b,(select ss_year, ss_asses_pno, ss_categ, count(*) completed from hrps.t_survey_status where ss_year = :fy and ss_status = 'SE' and ss_del_tag = 'N' and ss_app_tag = 'AP' and ss_wfl_status = 3  group by ss_year, ss_asses_pno, ss_categ) c where a.ss_year = b.ss_year(+)  and a.ss_asses_pno = b.ss_asses_pno(+) and a.ss_categ = b.ss_categ(+) and a.ss_year = c.ss_year(+) and a.ss_asses_pno = c.ss_asses_pno(+) and a.ss_categ = c.ss_categ(+) and a.ss_asses_pno =:pno group by A.ss_year,A.ss_asses_pno "
            qry1.Connection = conHrps
            qry1.Parameters.Clear()
            qry1.Parameters.AddWithValue("pno", pno.ToString)
            qry1.Parameters.AddWithValue("fy", ViewState("FY").ToString)
            'qry1 += "and upper(ss_categ)=upper(irc_code) and irc_type='360RL' and ss_year='2020' group by SS_CATEG,irc_desc"
            Dim re1 As String = String.Empty
            Dim cont1 As Integer = 0
            Dim d1 = getRecordInDt(qry1)
            ...
            If Not d Is Nothing Then
                ...
                lblnor.Text = "Your behaviour score is based on the responses of <b>" & cont & " individuals( Manager/Superior, Peer, People you lead)</b>. "
            Else
                cont = 0
                're = "Internal stakeholder - 0, Manager/Superior - 0, Peer - 0, Subordinates - 0, Self - 0"
                re = " Manager/Superior, Peer, People you lead"
    ...
    Public Sub getScore(pno As String)
        Try
            ...
            qry.CommandText = "select decode(upper(ss_categ),upper('Self'),'Self',upper('MANGR'),'Manager',upper('PEER'),'Peers',upper('intsh') "
            qry.CommandText += ",'Internal stakeholders',upper('ropt'),'People you lead') ss_categ,sum(ss_q1_a),sum(ss_q1_b),sum(ss_q1_c),sum(ss_q1_d),round(sum(ss_q1_a)/count(*),3) A,"
            qry.CommandText += "round(sum(ss_q1_b)/count(*),3) B,round(sum(ss_q1_c)/count(*),3) C,round(sum(ss_q1_d)/count(*),3) D"
            qry.CommandText += ",count(*) No_Records,upper(ss_categ) categ from t_survey_status where ss_asses_pno =:pno and ss_wfl_status ='3' and ss_year=:fy and ss_status = 'SE' and ss_del_tag = 'N' and ss_app_tag = 'AP' and upper(ss_categ) not in ('INTSH') group by ss_categ"
            qry.Connection = conHrps
            qry.Parameters.Clear()
            qry.Parameters.AddWithValue("pno", pno.ToString)
            qry.Parameters.AddWithValue("fy", ViewState("FY").ToString)
            ...
            If Not d Is Nothing Then
                ...
                '            sql += " where ss_asses_pno ='" & pno & "' and ss_wfl_status ='3' and upper(ss_categ) in ('PEER','ROPT') and ss_year='2020' and ss_status = 'SE' and ss_del_tag = 'N' and ss_app_tag = 'AP'"
                ...
                Dim sql1 As New OracleCommand()
                sql1.CommandText = "select ac.ss_asses_pno, round(sum(a)/count(*),3) A, round(sum(b)/count(*),3) B,round(sum(c)/count(*),3) C,round(sum(d)/count(*),3) D from  (SELECT ss_asses_pno,ss_categ,round(SUM(ss_q1_a) / COUNT(*),3) a,round(SUM(ss_q1_b) / COUNT(*),3) b,round(SUM(ss_q1_c) / COUNT(*),3) c,round(SUM(ss_q1_d) / COUNT(*),3) d, COUNT(*) no_records FROM hrps.t_survey_status WHERE ss_wfl_status = '3' AND ss_year = :fy AND ss_status = 'SE' AND ss_del_tag = 'N' AND ss_app_tag = 'AP' AND upper(ss_categ) not in ('SELF','INTSH')  AND ss_asses_pno = :pno GROUP BY ss_asses_pno,ss_categ) ac group by ac.ss_asses_pno "

                sql1.Connection = conHrps
                sql1.Parameters.Clear()
                sql1.Parameters.AddWithValue("pno", pno.ToString)
                sql1.Parameters.AddWithValue("fy", ViewState("FY").ToString)

                Dim dt4 = getRecordInDt(sql1)
                ...
                'sql1 += " where ss_asses_pno ='" & pno & "' and ss_wfl_status ='3' and upper(ss_categ) <> 'SELF' and ss_year='2020' and ss_status = 'SE' and ss_del_tag = 'N' and ss_app_tag =s 'AP'"
