...
Partial Class AddRespondent
    ...
    Private Sub getFy()
        Try
            ...
            If g.Rows.Count > 0 Then
                ...
                Session("FYforNamePopup") = g.Rows(0)("IRC_DESC").ToString()
    ...
    Public Sub getsrlno()
        Try
            ...
            If dtsrl.Rows.Count > 0 Then
                ...
                Session("CycleforNamePopup") = dtsrl.Rows(0)("IRC_DESC").ToString()
            End If
        Catch ex As Exception
    ...
    Protected Sub btnAddP_Click(sender As Object, e As EventArgs)
        Try
            If txtpnoI.Text.Trim() <> "" And DropDownList1.SelectedValue <> "Select assessors category" Then
                ...
                If val = "" Then
                    'Added by TCS on 13122023, To verify whether category is selected or not
                    If String.IsNullOrEmpty(Convert.ToString(DropDownList1.SelectedValue)) Or String.IsNullOrWhiteSpace(Convert.ToString(DropDownList1.SelectedValue)) Then
                        ShowGenericMessageModal(CommonConstants.AlertType.error, " Please Select the Category...!")
                        Exit Sub
                    End If
                    'End
                    If Not IsManagerBelongSameFunction(assesor, pno) Then
                        ShowGenericMessageModal(CommonConstants.AlertType.error, "Your skip level manager must belong to your function")
                        Exit Sub
                    End If
    ...
    Protected Sub btnaddnorgI_Click(sender As Object, e As EventArgs)
        Try
            ...
            If val = "" Then
                If String.IsNullOrEmpty(Convert.ToString(ddlrole.SelectedValue)) Or String.IsNullOrWhiteSpace(Convert.ToString(ddlrole.SelectedValue)) Then
                    ShowGenericMessageModal(CommonConstants.AlertType.error, " Please Select the Category...!")
                    Exit Sub
                End If
    ...
    Public Sub SaveData(ByVal role As String, ByVal pno As String, ByVal name As String, ByVal desg As String, ByVal org As String, ByVal email As String, ByVal lvl As String, ByVal orgtype As String, assespno As String)
        ...
        Try
            ...
            If role.ToString = "MANGR" Then
                Dim iskipMngrMax = checkSkipMAnagerMaxSelection(pno, name)
            ...
            If role.ToString = "INTSH" Then
                If Not isINTSHSelectionValid(pno) Then
                    Dim errorMsg = pno & " (" & name & "), "
                    ShowGenericMessageModal(CommonConstants.AlertType.error, errorMsg & " who has been identified as an internal stakeholder, has crossed the maximum limit of response as an internal stakeholder. You may wish to choose a different respondent as an Internal Stakeholder.")
                    Exit Sub
                End If
            End If
  ...
  System.Web.Services.WebMethod()>
    Public Shared Function SearchPrefixesForApprover(ByVal prefixText As String, ByVal count As Integer) As List(Of String)
        ...
        Try
            ...
            'Added by TCS on 27112025 to add Year and Cycle Filter
    ...
    Protected Sub Submit()
        Try
            ...
            If Session("label").Equals("I2") Then
                'Commented and added by TCS on 30112022, Changing query to fix manager selection (reporting perno column)
    ...
    Public Function ChkValidationmaxstake1() As String
        ...
        Return status.TrimEnd(",")
    End Function
    Public Function checkSkipMAnagerMaxSelection(pno As String, name As String) As String
        Dim errorMsg As String = String.Empty
        Dim cmd As New OracleCommand()
        Try
            If Not IsSkipManagerSelectionValid(pno) Then
                errorMsg += name & " (" & pno & "), "
            End If
        Catch ex As Exception
        End Try
    ...
    Public Function IsSkipManagerSelectionValid(ByVal skipMgrPerno As String) As Boolean
        ...
        Return isValid
    End Function
    Public Function checkINTSHMaxSelection(assespno As String) As String
        Dim errorMsg As String = String.Empty
        Dim cmd As New OracleCommand()
        Try
            ...
            cmd.Parameters.Clear()
            cmd.Parameters.AddWithValue("SS_SRLNO", ViewState("SRLNO").ToString())
            cmd.Parameters.AddWithValue("SS_ASSES_PNO", assespno)
    ...
    Public Function getMaxINTSHIdentifiedCount(ByVal level As String) As Integer
        ...
        Return count
    End Function
    Public Function IsManagerBelongSameFunction(assessePno As String, pno As String) As Boolean
        Dim isTrue = False
        Try
            Dim cmd As New OracleCommand()
            cmd.CommandText = " select ema_perno from hrps.t_emp_master_feedback360 where EMA_EXEC_HEAD in (select distinct EMA_EXEC_HEAD from hrps.t_emp_master_feedback360 where ema_perno= :ASSESSE and EMA_YEAR= :EMA_YEAR and EMA_CYCLE= :EMA_CYCLE) and ema_perno =  :ema_perno and ema_year=:ema_year and ema_cycle=:ema_cycle and case WHEN (select EMA_EQV_LEVEL from hrps.t_emp_master_feedback360 where ema_year=:ema_year and ema_cycle=:ema_cycle and ema_perno= :ASSESSE) IN ('I4','I5','I6') THEN CASE WHEN EMA_EQV_LEVEL <> 'I1' THEN 1 ELSE 0 END ELSE 1 END = 1 "
            cmd.Connection = conHrps
            cmd.Parameters.Clear()
            cmd.Parameters.AddWithValue("ASSESSE", assessePno)
            cmd.Parameters.AddWithValue("EMA_PERNO", pno)
