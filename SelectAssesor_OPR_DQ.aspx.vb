...
''' </summary>
Partial Class SelectAssesor_OPR_DQ
    ...
    Public Sub loadLoggedInUserIDAndDomainIntoSession()
        ...
        '    '    strUserID = "150006"
        'End If
        'strUserID = "910212" '"153342" '"910271" '910212
        ''strUserID = "269139"
            ...
            If dtchkval.Rows.Count = 0 Then
                If ViewState("type") = "TSL" Then
                    ...
                    If Session("label").Equals("I2") Then
                        strself.CommandText += " from hrps.t_emp_master_feedback360 a,hrps.t_emp_master_feedback360 b where b.ema_perno in (a.EMA_REPORTING_TO_PNO,a.ema_dotted_pno,a.EMA_PERS_EXEC_PNO) and a.ema_perno=:ema_perno AND a.EMA_YEAR=:SS_YEAR AND a.EMA_CYCLE =:SS_SRLNO"
                    Else
                        strself.CommandText += " from hrps.t_emp_master_feedback360 a,hrps.t_emp_master_feedback360 b where b.ema_perno in (a.EMA_REPORTING_TO_PNO,a.ema_dotted_pno) and a.ema_perno=:ema_perno AND a.EMA_YEAR=:SS_YEAR AND a.EMA_CYCLE =:SS_SRLNO"
                    End If
            ...
            If Session("label") = "I3" Or Session("label") = "I2" Or Session("label") = "I1" Then
                ...
                If dtchkvalfetch.Rows.Count = 0 Then
                    If ViewState("type") = "TSL" Then
                        ...
                        ' Start WI368  by Manoj Kumar on 30-05-2021 add emp class column for officer only)
                        strself.CommandText += " =(select ema_reporting_to_pno from hrps.t_emp_master_feedback360 where ema_perno=:ema_perno and ema_year=:SS_YEAR and ema_cycle=:SS_SRLNO ) and ema_year=:SS_YEAR and ema_cycle=:SS_SRLNO and EMA_EQV_LEVEL=:EMA_EQV_LEVEL"      'WI368 add column for officer only
                        'End by Manoj Kumar on 30-05-2021

                    Else
                        strself.CommandText += " select  ema_perno, ema_ename,EMA_EMPL_SGRADE EMA_EMPL_PGRADE,EMA_DESGN_DESC,EMA_DEPT_DESC,EMA_EMAIL_ID,'ORG' SSTYPE,'' CATEG_SHORT,'' CATEG_FULL   from hrps.t_emp_master_feedback360 where ema_reporting_to_pno "
                        ' Start WI368  by Manoj Kumar on 30-05-2021 add emp class column for officer only)
                        strself.CommandText += " =(select SED_REPORTING_TO_PNO  from HRPS.t_survey_emp_deputation where SED_PERNO=:ema_perno and SED_YEAR=:SS_YEAR and SED_CYCLE=:SS_SRLNO ) and ema_year=:SS_YEAR and ema_cycle=:SS_SRLNO and EMA_EQV_LEVEL=:EMA_EQV_LEVEL"      'WI368 add column for officer only
                        'End by Manoj Kumar on 30-05-2021
    ...
    Public Sub bindFinalGrid()
        ...
        Try
            ...
            If dtchkval.Rows.Count > 0 Then
                ...
                cmd.CommandText += ",'ORG' SSTYPE,'' CATEG_SHORT,decode(SS_WFL_STATUS,'2','Pending','3','Completed') SS_WFL_STATUS from hrps.t_emp_master_feedback360 , t_survey_status  where  SS_PNO=ema_perno(+) and SS_ASSES_PNO =:SS_ASSES_PNO and SS_STATUS ='SE' and SS_SRLNO=:SS_SRLNO"
  ...
  System.Web.Services.WebMethod()>
    Public Shared Function SearchPrefixesForApprover(ByVal prefixText As String, ByVal count As Integer) As List(Of String)
        Dim ob As New SelectAssesor_OPR_DQ
        Try

            Dim cmd As New OracleCommand
            cmd.CommandType = Data.CommandType.Text

            cmd.CommandText = " select ema_ename ||'('|| ema_perno ||')' EName from hrps.t_emp_master_feedback360 where (ema_perno like  :ema_perno or upper(ema_ename) like "
            ' Start WI368  by Manoj Kumar on 30-05-2021 add emp class column for officer only)
    ...
    Protected Sub txtPnoNoOpr_TextChanged(sender As Object, e As EventArgs)
        Try
            Dim words As String() = txtpnoopr.Text.Trim().Split(New Char() {"("c})
    ...
    Protected Sub txtpnoI_TextChanged(sender As Object, e As EventArgs)
        Try
            Dim words1 As String() = txtpnoI.Text.Trim().Split(New Char() {"("c})
            Dim pno = words1(1).Replace(")", "").ToString

            'Dim pno = txtpnoI.Text.Trim()
            Dim strself As New OracleCommand()
            strself.CommandText = "select ema_perno, ema_ename,EMA_EMPL_SGRADE EMA_EMPL_PGRADE,EMA_DESGN_DESC,EMA_DEPT_DESC,EMA_EMAIL_ID from hrps.t_emp_master_feedback360 where "
            ' Start WI368  by Manoj Kumar on 31-05-2021 add emp class column for officer only)
            strself.CommandText += " ema_perno=:pno and EMA_YEAR=:EMA_YEAR AND EMA_CYCLE=:EMA_CYCLE "
    ...
    Protected Sub btnaddmanager_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            If txtpnoopr.Text.Trim() <> "" Then
                ...
                Dim words1 As String() = txtpnoopr.Text.Trim().Split(New Char() {"("c})
                Dim pno = words1(1).Replace(")", "").ToString
                ...
                If val = "" Then
                    CType(ViewState("GvManager"), DataTable).Rows.Add(pno, name, lvl, desg, dept, email, "ORG", "MANGR", "Manager/Superior")   ' Added by Manoj Kumar on 25-05-2021
    ...
    Public Function ValidPerno() As Boolean
        ...
        Try
            ...
            ocmd.CommandText = "select * from hrps.t_emp_master_feedback360 where ema_perno ='" + Session("USER_ID").ToString + "' and trunc(ema_step1_stdt) <= trunc(sysdate)"
            ocmd.CommandText += "  and trunc(ema_step1_enddt) >= trunc(sysdate)  AND EMA_YEAR='" + ViewState("FY").ToString + "' AND EMA_CYCLE='" + ViewState("SRLNO").ToString + "' and ema_perno not in (select ee_pno from hrps.t_emp_excluded a  WHERE a.ee_cl='2' AND a.ee_year='2021')"
            Dim vc = getRecordInDt(ocmd, conHrps)
            If vc.Rows.Count > 0 Then
                isvalid = True
            Else
