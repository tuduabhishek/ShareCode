...
Partial Class FeedbackAdm_Adm
    ...
    End Sub
    Private Sub FeedbackAdm_Adm_Init(sender As Object, e As EventArgs) Handles Me.Init
 ...
 "document.body.style.cursor = 'wait'; this.value='Please wait...'; this.disabled = true; " + ClientScript.GetPostBackEventReference(Button1, String.Empty) + ";")
    ...
    Public Function GetPno(pernr As String) As String
        Try
            ...
            q.CommandText = "Select ema_perno,ema_ename from t_ir_adm_grp_privilege,hrps.t_emp_master_feedback360 where igp_group_id IN ('360DF_SA','360DF_LD','360DF_MD')  and IGP_STATUS ='A' "
            q.CommandText += "  and ema_perno=IGP_user_id and IGP_user_id='" & pernr & "' and EMA_COMP_CODE='1000' and ema_year='" + ViewState("FY") + "' and EMA_CYCLE='" + ViewState("SRLNO") + "'"
            q.CommandText += " UNION select ema_bhr_pno, EMA_BHR_NAME ema_ename  from hrps.t_emp_master_feedback360  where ema_bhr_pno ='" & pernr & "' AND EMA_CYCLE='" & ViewState("SRLNO").ToString & "' AND EMA_YEAR='" & ViewState("FY").ToString & "' and rownum=1"
            q.CommandText += " UNION select IGP_user_id, '' ema_ename from hrps.t_ir_adm_grp_privilege where igp_group_id IN ('360DF_SA','360DF_LD','360DF_MD')  "
            q.CommandText += " and IGP_STATUS ='A' and IGP_user_id='" & pernr & "'"
            'End
    ...
    Public Sub bindPendingRecord(Asspno As String)
        Try
            ...
            sql += "where ss_asses_pno=ema_perno and SS_YEAR=ema_year and ss_srlno=ema_cycle  and SS_YEAR =:SS_YEAR and SS_DEL_TAG='N'  and ss_app_tag='AP' and ss_wfl_status<>'1' and ema_comp_code='1000'"

            If pno.Length() > 6 Then
                mail = pno.ToUpper().Trim
                sql += " and TRIM(upper(SS_EMAIL))||SS_INTSH_OTP=:SS_INTSH_OTP and SS_SRLNO=:SS_SRLNO"
            Else
                sql += " and SS_PNO=:SS_PNO and SS_SRLNO=:SS_SRLNO"
            End If
            sql += " and ss_wfl_status in ('2','3','9')"
            If Asspno = "" Then
            Else
            ...
            '    sql += "  and EMA_EQV_LEVEL in ('I1','I2','TG') "
            'End If

            'If Session("LOGINID") = "HRBP" Then
            '    sql += "  and EMA_BHR_NO = '" + Session("USER_ID") + "'"
            'End If
            'End
    ...
    Protected Sub lbtnShow_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbtnShow.Click
        ...
        If Session("LOGINID") <> "SA" Then
            If Not CheckTransactionValid() Then
                txtRespondent.Text = String.Empty
                ShowGenericMessageModal(CommonConstants.AlertType.error, "You are not authorised to respond for entered personal number")
                Exit Sub
            End If
        End If
        'End
    ...
    Private Sub populateQuestion(ByVal pno As String)
        ...
        Try
            ...
            End If
            ls_pno.CommandText = "select SS_QTEXT from hrps.t_survey_question q where q.ss_qlevel=:ss_qlevel and q.ss_year=:ss_year and q.ss_cycle=:ss_cycle order by q.ss_srl_no"
            ls_pno.Connection = conHrps
            ls_pno.Parameters.Clear()
    ...
    Public Function CheckTransactionValid() As Boolean
        ...
        Try
            ...
            ElseIf Session("LOGINID") = "MD" Then
                strQuery = "SELECT EMA_PERNO FROM T_EMP_MASTER_FEEDBACK360 WHERE EMA_YEAR= :YEAR AND EMA_CYCLE= :CYCLE AND EMA_PERNO = :PERNO AND EMA_PERNO IN ('119628')"
            ...
            ocmd1.Parameters.AddWithValue("PERNO", txtRespondent.Text.Trim().ToString)
            If Session("LOGINID") = "HRBP" Then
    ...
    Public Function CheckAuthentication(pno As String, pageName As String) As Boolean
        ...
        Try
            ...
            q.CommandText += " UNION SELECT DISTINCT EMA_BHR_PNO, EMA_BHR_NAME ema_ename,IPG_PROG_NAME,IGP_GROUP_ID  FROM T_EMP_MASTER_FEEDBACK360, t_ir_adm_grp_privilege, t_ir_grp_pgm_privilege, T_IR_ADM_PROGRAM WHERE EMA_YEAR='" + ViewState("FY") + "' AND EMA_CYCLE='" + ViewState("SRLNO") + "' AND IGP_GROUP_ID = GPP_GROUP_ID AND GPP_PROG_ID = IPG_PROG_ID and IGP_USER_ID = EMA_BHR_PNO AND IGP_STATUS = 'A' AND IGP_USER_ID = '" & pno & "' AND IGP_GROUP_ID = '360DF_HRBP' "
            Dim p = getDataInDt(q)
            If p.Rows.Count > 0 Then
    ...
    Public Function CheckUserRole() As Boolean
        ...
        Try
            ...
            Dim strAdmin, strLD, strHRBP, strMD As String
            ...
            strMD = "select IGP_user_id from hrps.t_ir_adm_grp_privilege where igp_group_id ='360DF_MD'"
            strMD += " and IGP_STATUS ='A' and IGP_user_id='" & Session("ADM_USER").ToString() & "'"
            'End If
            ...
            Dim cmdMD = New OracleCommand(strMD)
            Dim dtMD = getRecordInDt(cmdMD, conHrps)
            If dtMD.Rows.Count > 0 Then
                t = True
                lblname.Text = "MD Admin"
                Session("LOGINID") = "MD"
                Return t
            End If
        Catch ex As Exception
