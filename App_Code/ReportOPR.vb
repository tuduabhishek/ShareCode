...
Public Class ReportOPR
    ...
    Public Sub GenerateReport(ByVal perNo As String, ByVal yr As String, ByVal cycle As String)
        ...
        If f.Rows.Count > 0 Then
            ...
            End If
            Dim Str As String = ""
            Str += " Select * from hrps.V_ALL_QUESTIONS_T_2023I46  where EMA_PERNO = '" + perNo + "' and ss_year =" & yr & " and ss_srlno =" & cycle & "  ORDER BY ss_desc,ss_categ,ss_qcode "
        ...
        Using ms As New MemoryStream()
            ...
            'Dim writer As PdfWriter = PdfWriter.GetInstance(doc, New FileStream(DestinationLoc, FileMode.Create))
            ...
            doc.Add(New Chunk(" ""Evolve""", FontFactory.GetFont("Helvetica Neue", 12, Font.BOLD, New iTextSharp.text.BaseColor(0, 41, 96))))
            doc.Add(New Chunk("  – team building" & vbLf, FontFactory.GetFont("Helvetica Neue", 12)))
            '#End Region
            ...
            cell6 = ImageCell("PatchI4.jpg ", 73.0F, PdfPCell.ALIGN_LEFT)
            ...
            cell7 = ImageCell("PatchI5.jpg ", 73.0F, PdfPCell.ALIGN_LEFT)
            ...
            table.SetWidths(New Single() {4.0F, 2.0F, 2.0F, 2.0F, 2.0F})
            ...
            Dim tablefd1 As PdfPTable = New PdfPTable(5)
            tablefd1.WidthPercentage = 100.0F
            tablefd1.SetWidths({4.0F, 2.0F, 2.0F, 2.0F, 2.0F})
            ...
            Dim tablefd2 As PdfPTable = New PdfPTable(5)

            tablefd2.WidthPercentage = 100.0F
            tablefd2.SetWidths({4.0F, 2.0F, 2.0F, 2.0F, 2.0F})
            ...
            Dim tablefd3 As PdfPTable = New PdfPTable(5)

            tablefd3.WidthPercentage = 100.0F
            tablefd3.SetWidths({4.0F, 2.0F, 2.0F, 2.0F, 2.0F})
            ...
            Dim phPage10 As Phrase = New Phrase("Team Building     ", FontFactory.GetFont("calibri", 13, 1))
            ...
            Dim tablefd4 As PdfPTable = New PdfPTable(5)
            tablefd4.WidthPercentage = 100.0F
            tablefd4.SetWidths({4.0F, 2.0F, 2.0F, 2.0F, 2.0F})
            ...
            strengthQry = " Select distinct ss_categ,SS_Q2_A from t_survey_status df , t_survey_response fd where  df.ss_pno=fd.ss_pno And df.ss_asses_pno=fd.ss_asses_pno And fd.ss_serial = df.ss_srlno  And fd.ss_year = df.ss_year And df.ss_asses_pno='" & perNo & "' and df.ss_year='" & yr.ToString & "' and df.ss_srlno='" & cycle.ToString & "' and df.SS_RPT_FLAG='Y'"
            ...
            improvementQry = " select distinct df.ss_categ, df.SS_Q2_b from t_survey_status df , t_survey_response fd where  df.ss_pno=fd.ss_pno and df.ss_asses_pno=fd.ss_asses_pno and fd.ss_serial = df.ss_srlno  and fd.ss_year = df.ss_year and df.ss_asses_pno='" & perNo & "'  and df.ss_year='" & yr.ToString & "' and df.ss_srlno='" & cycle.ToString & "' and df.SS_RPT_FLAG='Y'"
            ...
            Dim items As String() = {"•   There are two categories of respondent: Manager and Internal Stakeholder.",
                                 "•   Manager has 33% weightage and Internal Stakeholder has 67%.",
            ...
            cell = ImageCell("EX1IL46.jpg", 70.0F, PdfPCell.ALIGN_CENTER)
            ...
            'cell = ImageCell("Improvement.png", 115.0F, PdfPCell.ALIGN_CENTER)
            cell = ImageCell("EX2IL46.jpg", 70.0F, PdfPCell.ALIGN_CENTER)
    ...
    Private Function GetCellForBorderedTable(ByVal phrase As Phrase, ByVal align As Integer, ByVal color As BaseColor) As PdfPCell
        ...
        cell.MinimumHeight = 60.0F
        Return cell
    End Function

    Public Shared Function AccountibilityData(ByVal perNo As String, ByVal year As String, ByVal cycle As String, ByVal allResponseData As DataTable, ByVal level As String) As DataTable
    ...
    Public Function DisplayOverallData(ByVal perNo As String, ByVal year As String, ByVal cycle As String, ByVal level As String, ByVal dtAll As DataTable) As DataTable
        ...
        strQuery = " Select * from hrps.V_ALL_I1_I6_T_2023I46  where EMA_PERNO = '" + perNo + "' and ss_year =" & year & " and ss_srlno =" & cycle & "  ORDER BY 1 "
        'End
        ...
        If dtOverallData.Rows.Count > 0 Then
            ...
            dtOverall.Rows.Add("Team Building")
...
Class MyFooter
    ...
    Private ffont As iTextSharp.text.Font = New iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.UNDEFINED, 5, iTextSharp.text.Font.ITALIC)

    Public Overrides Sub onEndPage(ByVal writer As PdfWriter, ByVal document As Document)
        Dim footer As Paragraph = New Paragraph("Thank You", FontFactory.GetFont(FontFactory.TIMES, 10, iTextSharp.text.Font.NORMAL))
        footer.Alignment = Element.ALIGN_RIGHT
        Dim pageSize As iTextSharp.text.Rectangle = document.PageSize


        Dim headerTbl As PdfPTable = New PdfPTable(1)
        headerTbl.TotalWidth = 100.0F
        headerTbl.HorizontalAlignment = Element.ALIGN_RIGHT
        Dim phyPathH As String = System.AppDomain.CurrentDomain.BaseDirectory & "" & "images"
        Dim DestinationLocH As String = phyPathH & "\" & "Logo.jpg"
        Dim imageH As iTextSharp.text.Image = iTextSharp.text.Image.GetInstance(DestinationLocH)
        imageH.ScalePercent(60.0F)
        Dim cellH As PdfPCell = New PdfPCell(imageH)
        cellH.Border = 0
        cellH.PaddingLeft = 0
        headerTbl.AddCell(cellH)
        headerTbl.WriteSelectedRows(0, -1, pageSize.GetLeft(490), pageSize.GetTop(10), writer.DirectContent)


        'Dim footerTblPageNumber As PdfPTable = New PdfPTable(1)
        'footerTblPageNumber.TotalWidth = 10000.0F
        'footerTblPageNumber.HorizontalAlignment = Element.ALIGN_CENTER

        'Dim phraseFooter = New Phrase("Page 1")
        'Dim cell1 As PdfPCell = New PdfPCell(footer)
        'cell1.Border = 0
        'cell1.PaddingLeft = 0
        'cell1.HorizontalAlignment = Element.ALIGN_RIGHT
        'footerTblPageNumber.AddCell(cell1)
        'footerTblPageNumber.WriteSelectedRows(0, -1, pageSize.GetLeft(490), pageSize.GetTop(750), writer.DirectContent)
        Dim pageNumber As Integer = 0
        pageNumber = writer.CurrentPageNumber
        Dim pageNumberText As String = String.Empty

        If pageNumber <> 1 Then
            pageNumberText = "Page " + (pageNumber - 1).ToString
            Dim footer1 As Paragraph = New Paragraph(pageNumberText, FontFactory.GetFont("calibri", 10, iTextSharp.text.Font.NORMAL, BaseColor.GRAY))
            footer1.Alignment = Element.ALIGN_CENTER
            Dim footerTbl1 As PdfPTable = New PdfPTable(1)
            footerTbl1.TotalWidth = 300
            footerTbl1.HorizontalAlignment = Element.ALIGN_CENTER
            Dim cell1 As PdfPCell = New PdfPCell(footer1)
            cell1.Border = 0
            cell1.PaddingLeft = 10
            footerTbl1.AddCell(cell1)
            footerTbl1.WriteSelectedRows(0, -1, 274, 35, writer.DirectContent)
        End If


        Dim footerTbl As PdfPTable = New PdfPTable(1)
        footerTbl.TotalWidth = 1000.0F
        footerTbl.HorizontalAlignment = Element.ALIGN_CENTER




        Dim phyPath As String = System.AppDomain.CurrentDomain.BaseDirectory & "" & "images"
        Dim DestinationLoc As String = phyPath & "\" & "Patch.jpg"
        Dim image As iTextSharp.text.Image = iTextSharp.text.Image.GetInstance(DestinationLoc)
        image.ScaleAbsolute(600.0F, 30.0F)
        Dim cell As PdfPCell = New PdfPCell(image)
        cell.Border = 0
        cell.PaddingLeft = 0
        'footerTbl.AddCell(cell1)
        footerTbl.AddCell(cell)
        footerTbl.WriteSelectedRows(0, -4, -2, 18, writer.DirectContent)
    End Sub
