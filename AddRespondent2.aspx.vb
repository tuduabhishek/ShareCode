...
Partial Class AddRespondent2
    ...
    Public Sub getsrlno()
        Try
            ...
            If conHrps.State = ConnectionState.Closed Then
                conHrps.Open()
            End If
            mycommand.CommandText = "select IRC_DESC from hrps.t_ir_codes where IRC_TYPE='SL360' and IRC_VALID_TAG='Y'"
        ...
        Try
            If ChkRole() Then
                ...
                If ChkRole1() Then
                    ...
                    'qryrespond.CommandText += " where decode(c.ema_eqv_level,'I2',c.ema_dotted_pno,c.ema_reporting_to_pno)=D.EMA_PERNO AND c.ema_perno=tab.SS_ASSES_PNO and c.ema_year=d.ema_year and c.ema_cycle=d.ema_cycle  and c.ema_year=:fy and c.ema_cycle=:SS_SRLNO AND ROWNUM=1 ),'') approver from (select distinct A.ema_perno SS_ASSES_PNO,a.ema_ename,a.ema_desgn_desc from "
                    qryrespond.CommandText += " where c.ema_reporting_to_pno=D.EMA_PERNO AND c.ema_perno=tab.SS_ASSES_PNO and c.ema_year=d.ema_year and c.ema_cycle=d.ema_cycle  and c.ema_year=:fy and c.ema_cycle=:SS_SRLNO AND ROWNUM=1 ),'') approver from (select distinct A.ema_perno SS_ASSES_PNO,a.ema_ename,a.ema_desgn_desc from "
                    'End
                    qryrespond.CommandText += " hrps.t_survey_status, hrps.t_emp_master_feedback360 a    where a.ema_perno = SS_ASSES_PNO and a.ema_year=ss_year and a.ema_cycle=ss_srlno  and a.EMA_EQV_LEVEL in('I1','I2','I3','I4','I5','I6') and a.ema_comp_code='1000' "
                    ' qryrespond.CommandText += " and ss_status='SE' 
                    qryrespond.CommandText += " And ss_year=:fy "
                    'qryrespond.CommandText += " And (ss_del_tag='N' or ss_del_tag is null) "
                    qryrespond.CommandText += " And SS_SRLNO =: SS_SRLNO and A.EMA_BHR_PNO=:ema_perno " ' and SS_ASSES_PNO='" & txtperno1.Text.Trim & "'"
                    ...
                    qryrespond.CommandText += " union select distinct A.ema_perno SS_ASSES_PNO,a.ema_ename,a.ema_desgn_desc from "
                    qryrespond.CommandText += " hrps.t_survey_status , hrps.t_emp_master_feedback360 a    where  a.ema_perno = SS_ASSES_PNO(+) and a.ema_year=ss_year(+) and a.ema_cycle=ss_srlno(+) and a.ema_comp_code='1000' and a.EMA_EQV_LEVEL in('I1','I2','I3','I4','I5','I6') "
                    ' qryrespond.CommandText += " and ss_status='SE'"
                    qryrespond.CommandText += " And A.EMA_year=:fy "
                    ' qryrespond.CommandText += ""And (ss_del_tag ='N' or ss_del_tag is null) " 
                    qryrespond.CommandText += " and  A.EMA_CYCLE=:SS_SRLNO and a.ema_perno<>:ema_perno" ' and SS_ASSES_PNO='" & txtperno1.Text.Trim & "') tab"
                    If ddlExecutive.SelectedValue <> "--Select--" Then
                        qryrespond.CommandText += " and A.ema_exec_head=:ema_exec_head1"
                        qryrespond.Parameters.AddWithValue("ema_exec_head1", ddlExecutive.SelectedValue)
                    End If
                    ...
                    End If
                    If txtperno1.Text.Trim <> "" Then
                        'Added by TCS on 20122023, To show the result for the user who does not open the page
                        'qryrespond.CommandText += " and ss_asses_pno=:ss_asses_pno1"
                        qryrespond.CommandText += " and A.ema_perno=:ss_asses_pno1"
                        qryrespond.Parameters.AddWithValue("ss_asses_pno1", txtperno1.Text.Trim.ToString)
                    End If
                    If txtBuhr.Text.Trim <> "" Then
                        qryrespond.CommandText += " and A.ema_bhr_pno=:ema_bhr_pno1"
                ...
                Else
                    ...
                    'Added by TCS on 20122023, To show the result for the user who does not open the page
                    qryrespond.CommandText += " union select distinct A.ema_perno SS_ASSES_PNO,a.ema_ename,a.ema_desgn_desc from "
                    qryrespond.CommandText += " hrps.t_survey_status , hrps.t_emp_master_feedback360 a    where  a.ema_perno = SS_ASSES_PNO(+) and a.ema_year=ss_year(+) and a.ema_cycle=ss_srlno(+) and a.ema_comp_code='1000' and a.EMA_EQV_LEVEL in('I1','I2','I3','I4','I5','I6') and A.EMA_BHR_PNO=:ema_perno "
                    qryrespond.CommandText += " And A.EMA_year=:fy "
                    qryrespond.CommandText += " and  A.EMA_CYCLE=:SS_SRLNO and a.ema_perno<>:ema_perno"
                    If ddlExecutive.SelectedValue <> "--Select--" Then
                        qryrespond.CommandText += " and A.ema_exec_head=:ema_exec_head1"
                        qryrespond.Parameters.AddWithValue("ema_exec_head1", ddlExecutive.SelectedValue)
                    End If
    ...
    Protected Sub bindself()
        Try
            ...
            For Each gv As GridViewRow In gvself.Rows
                ...
                cmdqry.CommandText = "select a.IRC_CODE,REGEXP_SUBSTR (a.IRC_DESC, '[^-]+', 1, 1) minmum, REGEXP_SUBSTR (a.IRC_DESC, '[^-]+', 1, 2) maximum,b.irc_desc from hrps.t_ir_codes a,hrps.t_ir_codes b "
    ...
    Public Sub BindAssesorGrid(pno As String)
        Try
            ...
            Dim cmdqry As New OracleCommand()
            cmdqry.CommandText = "select a.IRC_CODE,REGEXP_SUBSTR (a.IRC_DESC, '[^-]+', 1, 1) minmum, REGEXP_SUBSTR (a.IRC_DESC, '[^-]+', 1, 2) maximum,b.irc_desc irc_desc1,decode(a.irc_type,'360V2',decode(a.IRC_CODE,'PEER','Peers','ROPT','Subordinates',b.irc_desc),'360V3',decode(a.IRC_CODE,'PEER','Peer/Subordinate',b.irc_desc),decode(a.IRC_CODE,'INTSH','Peer/Subordinate/Internal Stackholder',b.irc_desc)) irc_desc from hrps.t_ir_codes a,hrps.t_ir_codes b "
    ...
    Public Sub PopDroupdown()
        Try
            ...
            If lvl = "I4" Or lvl = "I5" Or lvl = "I6" Then
                qry = "select IRC_CODE,decode(upper(IRC_DESC),'INTERNAL STAKEHOLDER','Internal Stakeholder/Peers/Subordinates',IRC_DESC) IRC_DESC from hrps.t_ir_codes where irc_type='360RL' and IRC_VALID_TAG='A' and IRC_CODE not in('SELF','ROPT','PEER') order by 2"
            End If
            If lvl = "I3" Then
                qry = "select IRC_CODE,decode(UPPER(IRC_DESC),'PEER','Peers/Subordinates',IRC_DESC) IRC_DESC from hrps.t_ir_codes where irc_type='360RL' and IRC_VALID_TAG='A' and IRC_CODE not in('SELF','ROPT') order by 2"
            End If
            If lvl = "I2" Then
                qry = "select IRC_CODE,decode(UPPER(IRC_DESC),'PEER','Peers',IRC_DESC) IRC_DESC from hrps.t_ir_codes where irc_type='360RL' and IRC_VALID_TAG='A' and IRC_CODE not in('SELF') order by 2"
            End If
  ...
  System.Web.Services.WebMethod()>
    ...
    Public Sub GetSelf(ByVal ema_per_no As String)
        Try
            If conHrps.State = ConnectionState.Closed Then
                conHrps.Open()
            End If
            ...
            c.Connection = conHrps
            c.Parameters.Clear()
            c.Parameters.AddWithValue("ema_perno", ema_per_no)
            c.Parameters.AddWithValue("SS_YEAR", ViewState("FY").ToString())
            c.Parameters.AddWithValue("SS_SRLNO", ViewState("SRLNO").ToString())
            Dim d = getDataInDt(c)
            If d.Rows.Count > 0 Then
                c = New OracleCommand()
                c.CommandText = " select * from hrps.t_survey_status where ss_pno=:ss_pno  and SS_YEAR=:SS_YEAR and SS_ASSES_PNO=:ss_pno and SS_SRLNO=:SS_SRLNO and SS_CATEG='Self'"
                c.Connection = conHrps
                c.Parameters.Clear()
                c.Parameters.AddWithValue("ss_pno", ema_per_no)
                c.Parameters.AddWithValue("SS_YEAR", ViewState("FY").ToString())
                c.Parameters.AddWithValue("SS_SRLNO", ViewState("SRLNO").ToString())
                Dim f = getDataInDt(c)
                If f.Rows.Count > 0 Then
                Else
                    SaveData("Self", d.Rows(0)("ema_perno").ToString(), d.Rows(0)("ema_ename").ToString(), d.Rows(0)("EMA_DESGN_DESC").ToString(), d.Rows(0)("EMA_DEPT_DESC").ToString(), d.Rows(0)("EMA_EMAIL_ID").ToString(), d.Rows(0)("EMA_EMPL_PGRADE").ToString(), "ORG", d.Rows(0)("ema_perno").ToString())
                End If
            End If
        Catch ex As Exception
        Finally
